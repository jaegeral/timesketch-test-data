...
for file in $(find ../EVTX-ATTACK-SAMPLES/ -type f -name "*.evtx"); do
    if [ -f "$file" ]; then
        ...
        sudo docker run -v /Users/jaegeral/scripts/:/data log2timeline/plaso log2timeline /data/timesketch-test-data/EVTX-ATTACK-SAMPLES-plaso/$relpath.plaso /data/$relpath
    fi
done